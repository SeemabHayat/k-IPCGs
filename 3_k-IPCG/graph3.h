 ...
 *      Author: shurbevski
 updated on on 12-1-2018 changes: function: normalize
 */
	...
	size_t find_components(	std::vector < vertexSet > &comps) const{
		...
		for (auto &v: this->vertices){
			...
			while (! qq.empty()){
				...
				for (const auto &u: this->inAdjList[s]){
					if (! reached[u]) qq.push_back(u);
				}
				for (const auto &u: this->outAdjList[s]){
					if (! reached[u]) qq.push_back(u);
				}
			}
...
//	} // find bi-components
	 ...
	 * given s and t
	 */
	size_t getPathEdges(const vertex &s, const vertex &t, unordered_set <edge> &thePath) const{
		size_t length = 0;
		thePath.clear();
	...
	*/
	 ...
	 *
	 */
	void get_leaves(unordered_set<vertex> &leafSet) const {//!
		leafSet.clear();
		for (const auto &v : this->vertices){
...
//		}
	...
	void pretty_print_tree(ostream *ofs = &(std::cout), size_t count = 0) const {
		size_t nn = this->getN();
		vector<edge> ve;
		for (const auto &e : this->edges) {
			if (e.first < e.second) ve.push_back(e);
