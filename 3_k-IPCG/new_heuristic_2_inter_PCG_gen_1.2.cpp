	...
	mt19937 &mt)
{
	...
	for (int i = 0; i <= num_leaves - 1; i++) {
		//rand_no = mt() % (340 - 200 + 1) + 200; //[200, 340]
		rand_no = mt() % 100 + 1; //[1, 50]
		VecWeight.push_back(rand_no);
	}
	for (int j = num_leaves; j <= ((2 * num_leaves) - 4); j++) {
		//rand_no = mt() % (240 - 150 + 1) + 150; //[150, 240]
		rand_no = mt() % 100 + 1; //[1, 20]
		VecWeight.push_back(rand_no);
	}
...
int main(int argc, char **argv) {
	...
	string sfoundPCGsFile, sNotfoundgraphFile, snum_PCG_round;
	...
	snum_PCG_round = foldername.str() + "/" + to_string(Graph_num_ver) +
		"num_PCG_round" + to_string(time_limit) + ".txt";
	ofstream ofsfoundPCGsFile, ofsNotfoundgraphFile, ofsnum_PCG_round;
	try {
	...
	try {
		ofsnum_PCG_round.open(snum_PCG_round);
	}
	catch (const exception& e) {
		cerr << "Couldn't open PCG round '" << snum_PCG_round
			<< "' for reading!" << endl;
		throw(e);
	}
	if (!ofsnum_PCG_round.is_open()) {
		cerr << "Error: PCG round found file is not initialized!" << endl;
		throw(-1);
	}

	ostream *osfoundPCGsFile = &ofsfoundPCGsFile;
	ostream *osNotfoundgraphFile = &ofsNotfoundgraphFile;
	ostream *osnum_PCG_round = &ofsnum_PCG_round;
	//opening tree file//
	ifstream ifsTree;
	try {
	...
	mt19937 mt;//better to use this
			   ...
			   //	//return;
			   //}
	*osfoundPCGsFile << "weight interval for generator: for non-leaves [1, 100] and for leaves [1, 100]" << endl;
	*osfoundPCGsFile << "-------------------------------------------------------------------------------------" << endl;
	...
	int num_pcg_per_round = 0;
	while (time_spent / CLOCKS_PER_SEC <= time_limit) {//while for time//
													   //A: creating input Nauty file for each round//

		MSG("~~~~~~~~~~NEXT ROUND ~~~~~~~~");
		cout << "~~~~~~~~~~NEXT ROUND ~~~~~~~~" << endl;
		cout << "num_pcg_per_round: " << num_pcg_per_round << endl;
		*osnum_PCG_round << num_pcg_per_round << endl;
		num_pcg_per_round = 0;
		++round_counter;
		string sInputNauty;
